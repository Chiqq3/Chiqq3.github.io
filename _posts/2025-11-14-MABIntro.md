---
layout: post
title: Multi-Armed Bandits: A Gentle Introduction
...
- Prior: $p(\theta_i)$ for each arm $i$
- Posterior update: $p(\theta_i | \mathcal{D}_t) \propto p(\mathcal{D}_t | \theta_i) p(\theta_i)$
- Action selection: Sample $\tilde{\theta}_i \sim p(\theta_i | \mathcal{D}_t)$ and choose $\arg\max_i \tilde{\theta}_i$
