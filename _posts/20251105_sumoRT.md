...
description: A comprehensive tutorial on creating realistic vehicular networks by combining SUMO mobility simulation with MATLAB ray tracing for wireless communication research
...
- 📶 **MATLAB Ray Tracing** for wireless channel modeling
...
{% include figure.liquid loading="eager" path="assets/img/sumo-raytracing-demo.png" class="img-fluid rounded z-depth-1" caption="Vehicular network simulation combining SUMO mobility with MATLAB ray tracing" %}
...
Want different vehicle types in your simulation? Create a vehicle type distribution file (`*.add.vtype.xml`) following the 3GPP TR 37.885 evaluation scenario {% cite 3gpp_tr37885 %}:
...
Let's place realistic base station (BS) locations using real cellular network data from [OpenCellID](https://opencellid.org/).
...
<details markdown="1">
<summary>📚 BibTeX Entries (Click to expand)</summary>

```bibtex
@techreport{3gpp_tr37885,
    author = {{3GPP}},
    title = {Study on Evaluation Methodology of New Vehicle-to-Everything (V2X) 
             Use Cases for {LTE} and {NR}},
